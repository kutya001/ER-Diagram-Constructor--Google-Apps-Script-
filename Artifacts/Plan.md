...
**✅ Критерий завершения фазы 1: [ВЫПОЛНЕНО]**
...
**✅ Критерий завершения фазы 2: [ВЫПОЛНЕНО]**
...
**✅ Критерий завершения фазы 3: [ВЫПОЛНЕНО]**
...
**✅ Критерий завершения фазы 4: [ВЫПОЛНЕНО]**
...
## Фаза 5: Визуальные улучшения и стабильность (Текущая, ~2–3 часа)

### 5.1 Интеллектуальная подсветка на холсте
- При выборе таблицы подсвечивать входящие/исходящие связи.
- Приглушать остальные таблицы для фокуса.
- Выделять конкретные соединяющие колонки (bold + background).

### 5.2 Стандартизация Drag-and-Drop
- Исправлена инициализация `dataTransfer` для Firefox.
- Синхронизирован порядок во всех модалках и правой панели.

### 5.3 Поле «Примечание» (Note)
- Добавлена поддержка поля `note` для таблиц во всех слоях (GS, API, UI).

### 5.4 Оптимистичный UI для колонок
- Мгновенная перепривязка колонок к новому ID таблицы при создании, предотвращая их временное исчезновение.
...
| Фаза | Что делает | Ожидаемое время | Приоритет | Статус |
|------|-----------|-----------------|-----------|--------|
| **0** | Backup | 5 мин | Обязательно | [x] |
| **1** | Критические баги | 30 мин | 🔴 Критично | [x] |
| **2** | Разделение Frontend | 2-3 часа | 🔴 Критично | [x] |
| **3** | Разделение Backend | 1-2 часа | 🟡 Важно | [x] |
| **4** | Улучшения качества | 1-2 часа | 🟡 Важно | [x] |
| **5** | UX и Визуализация | 2 часа | 🟢 Желательно | [/] |
